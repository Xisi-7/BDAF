...
def arg_parse():
    parser = argparse.ArgumentParser(description='Buffer-Guided Differentiated Learning for Adversarial Fine-Tuning')
    parser.add_argument('--seed', default=100, type=int, help='which seed the code runs on')
    parser.add_argument('--gpu', default='0', type=str, help='which gpu the code runs on')
    parser.add_argument('--dataset', default='animals10', choices=['cifar10', 'stl10', 'gtsrb','animals10'])
...
def classify(args, encoder):
    ...
    return best_clean_acc_t1,best_adv_acc_t1
...
def main():
    ...
    best_clean_acc_t1,best_adv_acc_t1 = classify(args, model)
    print('Clean downstream accuracy: %.4f%%' % (best_clean_acc_t1))
    print('Adv downstream accuracy: %.4f%%' % (best_adv_acc_t1))
